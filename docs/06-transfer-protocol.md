...
```
A → B : RekeyHello { conn_id, eph_pub_A }   A は接続開始側。conn_id と eph_pub は接続ごとに新規生成
B → A : RekeyHello { conn_id, eph_pub_B }

両者 : shared      = X25519(eph_priv, eph_pub_peer)
両者 : conn_transcript = SHA-256(conn_id || RekeyHello_A_raw || RekeyHello_B_raw)
...
`RekeyHello_A_raw` と `RekeyHello_B_raw` は、長さプレフィックスを除いた送信時の CBOR バイト列そのものとする。受信後に再エンコードしない。`conn_id` は接続ごとに A が新規生成する乱数で、B は同じ値を応答に入れる。これにより、同一セッション内での接続の使い回しや古い `conn_transcript` の再送を防ぐ。
...
ciphertext := ChaCha20-Poly1305(key = k_send_self_conn,
                                nonce = 0x000000 || direction_byte || counter (u64 BE),
                                aad = "",
                                plaintext)
```

- `direction_byte` は A から B が `0x00`、B から A が `0x01`
...
TLS を使わない理由は、鍵配送が BLE ハンドシェイクと接続ごとの Rekey で完了しており、証明書基盤も TLS のセッション再開も不要なためである。
...
| 0x08 | Abort | 双方向 | `{ transfer_id?, reason }` |
| 0x09 | Ping / 0x0A Pong | 双方向 | `{}`（無通信 15 秒で送出、30 秒で切断判定） |

`kind` は `file`、`directory`、`text` のいずれか。
ディレクトリは相対パス付きのファイル列に展開して items に並べる（パス区切りは `/` に正規化し、`..` を含むパスは受信側で拒否する）。
テキストは UTF-8 バイト列として Chunk で送り、ファイルと同じ Offer、Answer、SHA-256 検証を通す。`name` はテキストの場合だけ省略する。本文を Offer に含めないため、受信側の許可前に内容は渡らない。BleGatt を選んだ場合も同じプロトコルで転送できる。
...
1 セッションで進行中にできる転送は 1 つだけとする。双方が同時に Offer を開始した場合は、`session_device_id` が小さい端末の Offer を先に処理し、もう一方は完了まで送信を待つ。
...
プロセス終了もセッション終了とはみなさない。進行中の 1 転送については `03-architecture.md` の復元状態を保存し、アプリ再起動後に同じ `k_auth` で新しい Stream を認証して上記の再開手順へ戻る。
